...
CREATE TABLE categories (
    ...
    CONSTRAINT categories_name_not_blank CHECK (char_length(btrim(name)) BETWEEN 1 AND 100),
...
CREATE TABLE listings (
    ...
    CONSTRAINT listings_title_not_blank CHECK (char_length(btrim(title)) BETWEEN 1 AND 120),
    CONSTRAINT listings_description_length CHECK (char_length(description) <= 5000),
    CONSTRAINT listings_price_range CHECK (price_idr BETWEEN 1 AND 10000000000),
    ...
    CONSTRAINT listings_updated_after_created CHECK (updated_at >= created_at),
...
CREATE INDEX listings_seller_id_status_updated_at_idx ON listings (seller_id, status, updated_at DESC);

...
CREATE TABLE categories (
    ...
    CONSTRAINT categories_slug_nonblank CHECK (char_length(btrim(slug)) > 0),
    CONSTRAINT categories_slug_length CHECK (char_length(slug) BETWEEN 1 AND 50),
    CONSTRAINT categories_name_nonblank CHECK (char_length(btrim(name)) > 0),
...
CREATE TABLE listings (
    ...
    CONSTRAINT listings_title_nonblank CHECK (char_length(btrim(title)) BETWEEN 1 AND 120),
    ...
    CONSTRAINT listings_updated_at_order CHECK (updated_at >= created_at)
);

CREATE INDEX listings_status_created_at_id_idx ON listings (status, created_at DESC, id DESC);
CREATE INDEX listings_category_status_created_at_idx ON listings (category_id, status, created_at DESC);
CREATE INDEX listings_seller_status_updated_at_idx ON listings (seller_id, status, updated_at DESC);
CREATE INDEX listings_price_idr_id_idx ON listings (price_idr, id);
